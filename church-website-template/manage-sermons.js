...
async function loadSermons() {
  ...
  sermons.forEach(sermon => {
    ...
    item.innerHTML = `
      <div>
        ...
        <div class="admin-actions">
          ...
          <button
            class="btn danger delete-sermon"
            type="button"
          >
            Delete
          </button>
    ...
    const editButton = item.querySelector(".edit-sermon");
    const deleteButton = item.querySelector(".delete-sermon");
...
sermonForm.addEventListener("submit", async event => {
  ...
  if (!isEditing && sermonMp3.files.length === 0) {
  ...
  formData.append(
    "speaker",
    sermonSpeaker.value.trim()
  );
  formData.append(
    "scripture_passage",
    scripturePassage.value.trim()
  );
  formData.append(
    "service",
    sermonService.value
...
async function deleteSermon(id) {
  ...
  const response = await fetch(
    `/api/sermons?id=${encodeURIComponent(id)}`,
    {
      method: "DELETE"
    }
  );
